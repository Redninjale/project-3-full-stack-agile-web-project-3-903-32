...
const Menu = () => {
  ...
  const handleSaveMenu = (event) => {
    ...
    putMenu(
      ...
      parseFloat(document.getElementById("item_price").value).toFixed(2),
  ...
  const handleAddMenu = (event) => {
    ...
    const curr_price = parseFloat(
      document.getElementsByName("add_menu_price")[0].value
    ).toFixed(2);
    const curr_category =
      document.getElementsByName("add_menu_category")[0].value;
  ...
  const handleAddItem = (event) => {
    ...
    if (
      ...
      document.getElementById("newItemAmount").value = parseFloat(
        document.getElementById("newItemAmount").value
      ).toFixed(2);
  ...
  const handleChangeAmount = (index, event) => {
    ...
    if (event.keyCode === 13) {
      document.getElementsByName("editItemAmount")[index].value = parseFloat(
        document.getElementsByName("editItemAmount")[index].value
      ).toFixed(2);
      itemInventoryList[index].itemAmount =
        document.getElementsByName("editItemAmount")[index].value;
  ...
  async function putMenu(menu_id, menu_name, menu_price, menu_category) {
    try {
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/menu/" + menu_id,
        {
          method: "PUT",
          mode: "cors",
          headers: {
  ...
  async function getMenuInventory(menu_id) {
    try {
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/mijunc/" + menu_id,
        {
          method: "GET",
          mode: "cors",
        }
  ...
  async function deleteMenuInventory(menu_id, index) {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "Failed to delete menu inventory:",
          response.status,
          response.statusText
        );
      }
    ...
    try {
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/mijunc/" + menu_id,
        {
          ...
          body: JSON.stringify({
            itemID: itemInventoryList[index].itemID,
            itemAmount: itemInventoryList[index].itemAmount,
          }),
        }
      );
      if (response.ok) {
  ...
  async function getOutsideMenuInventory(menu_id) {
    try {
      // console.log(menu_id);
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/mijunc/outside/" + menu_id,
        {
          method: "GET",
          mode: "cors",
        }
    ...
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-2 border-gray-600 bg-gray-50 flex flex-col p-4 rounded">
      <div className="pb-4 px-4 flex flex-col relative">
        <div>
          ...
          <input
            className="my-2 px-2 mx-4 bg-gray-200 rounded"
            type="text"
            id="item_name"
          ...
          <input
            className="px-2 mx-4 bg-gray-200 rounded w-1/4"
            type="text"
            id="item_price"
          ...
          <button onClick={handleCloseModal}>
            <img
              ...
              alt="Close"
              className="h-[20px] absolute right-1 top-3"
            />
          </button>
        </div>
        <div>
          ...
          <select
            className="px-2 mx-4 bg-gray-200 rounded"
            id="item_category"
      ...
      <div className="mx-4 flex justify-between">
        ...
        >
          {confirmDelete ? "Confirm" : "Delete Menu Item"}
        </button>
      </div>
      <h1 className="text-center font-bold border-t-2 border-gray-500 mt-6 pt-6">
        Menu Ingredients
      </h1>
      <table>
        <thead>
            ...
            <tr key={index}>
              ...
              <td>
                <input
                  ...
                  onKeyDown={(e) => handleChangeAmount(index, e)}
                />
              </td>
              <td>
                <button
  ...
  const AddMenuModal = () => (
    <tr>
      <td className="p-4">
        <label htmlFor="add_menu_name">Menu Name:</label>
        <input
          className="bg-gray-200 rounded px-2 ml-2"
          type="text"
          name="add_menu_name"
        />
      </td>
      <td className="p-4">
        <label htmlFor="add_menu_price">Price: </label>
        <input
          className="bg-gray-200 rounded px-2 ml-2 w-3/5"
          type="number"
          name="add_menu_price"
        />
      </td>
      ...
      <td className="p-4 pl-0">
        <button name="add_menu_button" onClick={handleAddMenu}>
          Add
        </button>
      </td>
    </tr>
            ...
            <tbody>
              {menuItems.map((item, index) => (
                <tr key={index} className="bg-white border-b">
                  <td className="p-4 text-base text-gray-700 w-5/12">
                    {item.itemName}
                  </td>
                  <td className="p-4 text-base text-gray-700 w-1/4">
                    ${item.price}
                  </td>
                  <td className="p-4 text-base text-gray-700 w-1/3">
                    {item.category}
                  </td>
                  <td className="p-4 pl-0 text-base text-gray-700">
                    <button
                      id={index}
              ...
              {addMenuModal ? (
                ...
                <tr
                  key={-1}
                  className="bg-white border-b h-12 hover:cursor-pointer"
                  onClick={() => setAddMenuModal(true)}
                >
                  <td className="border-gray-700 border-dotted border-2 border-r-0"></td>
                  <td className="text-center text-2xl border-gray-700 border-dotted border-2 border-x-0 text-gray-700">
                    +
                  </td>
