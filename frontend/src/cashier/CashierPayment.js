      ...
      <div className="w-screen h-screen overflow-hidden flex flex-row gap-20 items-center justify-center bg-customMaroon">
                    ...
                    {order.map((item, index) => (
                      ...
                      >
                        <div className="ml-2 mt-2 mb-2">
                          {item.itemName} - ${item.price.toFixed(2)} x{" "}
                          {item.quantity}
                        </div>
                      </li>
                    ))}
        ...
        <div className="w-1/4 font-semibold">
          ...
          {showEmptyMessage ? (
            ...
            <button
              onClick={toCashierSubmit}
              disabled={order.length === 0 || paymentSubmitted || loading}
              className={`w-full overflow-y-auto py-2 my-2 rounded text-2xl ${
                order.length === 0
          ...
          <button
            onClick={toCashierBack}
            className="w-full overflow-y-auto py-2 my-2 bg-gray-50 hover:bg-gray-300 rounded text-2xl"
          >
            Back
          </button>
          <button
            onClick={toCashierCancel}
            className="w-full overflow-y-auto py-2 my-2 bg-red-600 hover:bg-red-800 text-white rounded text-2xl"
          >
            Clear Order
          </button>
        </div>
