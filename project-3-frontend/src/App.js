...
const clientId =
  "417248299016-d2tdli4igl731cienis995uaaeetb4vt.apps.googleusercontent.com";
  ...
  const getRole = async (email) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/employee/gmail/${email}`,
        {
          method: "GET",
          mode: "cors",
        }
  ...
  return (
    <div className="relative min-h-screen flex flex-col justify-between">
      <video
        ...
        src={imageMapping["video"]}
        ref={videoRef}
      ></video>
      <div className="flex flex-col items-center justify-center z-10">
        <h1
          className="text-white p-10 text-center text-9xl mb-8"
          style={{
        ...
        <div className="flex items-center justify-center flex-col">
          ...
          <div className="flex items-center mb-4">
            {" "}
            {/* Added a div to contain the cheeseburger image and button */}
            <img
              src={imageMapping["cheeseburgerImg"]}
              alt="Cheeseburger"
