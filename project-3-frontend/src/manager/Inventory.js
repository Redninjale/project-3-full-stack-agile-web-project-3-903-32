...
const Inventory = () => {
  ...
  async function putInventory({
    ...
    try {
      const response = await fetch(
        process.env.REACT_APP_BACKEND_URL + "/api/inventory/" + id,
        {
          method: "PUT",
          mode: "cors",
          headers: {
    ...
    try {
      await fetch(
        process.env.REACT_APP_BACKEND_URL +
          "/api/inventory/" +
          document.querySelector("select[name=restock_selector]").value +
          "/add",
        {
          method: "PUT",
          mode: "cors",
          ...
          body: JSON.stringify({
            ...
            add_stock: document.querySelector("input[name=restock_input]")
              .value,
          }),
        }
      )
  ...
  function handleShortage(event) {
    document.querySelector("select[name=restock_selector]").value =
      event.target.innerText.split("(")[0].slice(0, -1);
    console.log(
      (document.querySelector("input[name=restock_input]").value =
        event.target.innerText.split("(")[1].split(" ")[2].slice(0, -1))
    );
  }
    ...
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1/3 border-2 border-gray-600 bg-gray-50 flex flex-col p-4 rounded">
      <div className="pb-4 px-4 flex flex-col gap-2">
        <div className="flex justify-between">
          <h1 className="font-bold text-xl">Edit Inventory Item</h1>
        ...
        <form className="flex flex-col gap-2" onSubmit={submitEditInvetory}>
          <div className="flex justify-between">
            ...
            <input
              ...
              type="text"
              name="inventory_name"
              aria-label="inventory_name"
              defaultValue={editInventoryObject.name}
          ...
          <div className="flex justify-between">
            ...
            <input
              ...
              aria-label="inventory_stock"
              defaultValue={editInventoryObject.stock}
              className="border-2 border-gray-700 rounded px-2 py-1"
            />
          </div>
          <div className="flex justify-between">
            ...
            <input
              ...
              aria-label="inventory_minimum"
              defaultValue={editInventoryObject.minimum}
              className="border-2 border-gray-700 rounded px-2 py-1"
            />
          </div>
          <div className="flex justify-between">
            ...
            <input
              ...
              aria-label="inventory_capacity"
              defaultValue={editInventoryObject.capacity}
              className="border-2 border-gray-700 rounded px-2 py-1"
            />
          </div>
          <div className="flex justify-between">
            ...
            <input
              ...
              aria-label="inventory_location"
              defaultValue={editInventoryObject.location}
              className="border-2 border-gray-700 rounded px-2 py-1"
            />
          </div>
          <div className="flex justify-between">
            ...
            <input
              ...
              aria-label="inventory_supplier"
              defaultValue={editInventoryObject.supplier}
              className="border-2 border-gray-700 rounded px-2 py-1"
            />
          </div>
          <div className="mt-4 flex justify-between">
            ...
            >
              {confirmDelete ? "Confirm" : "Delete Inventory Item"}
            </button>
          </div>
        </form>
      </div>
  ...
  const AddInventoryModal = () => (
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1/3 border-2 border-gray-600 bg-gray-50 flex flex-col p-4 rounded">
      <div className="pb-4 px-4 flex flex-col gap-2">
        <div className="flex justify-between">
          <h1 className="font-bold text-xl">Add Inventory Item</h1>
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
              ...
              <div className="bg-white p-6 border-b border-gray-200">
                ...
                <form onSubmit={restockInventory} className="space-y-2">
                  <div>
                    ...
                    <select
                      required
                      name="restock_selector"
                      aria-label="restock_selector"
                  ...
                  <button
                    type="submit"
                    className="w-full p-2 bg-blue-700 text-white rounded hover:bg-blue-600"
                  >
                    Restock
                  </button>
                </form>
                ...
                <ul className="space-y-2">
                  {shortageItems.length > 0 ? (
                    shortageItems.map((item) => (
                      <li
                        key={item.name}
                        className="p-2 bg-red-100 border border-red-200 rounded"
                      >
                        <button
      ...
      {editModal && <EditInventoryModal />}
      {addModal && <AddInventoryModal />}
    </div>
  );
};
